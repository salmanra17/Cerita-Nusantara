...
class RegionDetailPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final String name = Get.arguments?['name'] ?? 'Jawa Barat';
    final ctrl = Get.find<StoryController>();

    return Scaffold(
      backgroundColor: const Color.fromARGB(255, 255, 255, 255),
      body: Obx(() {
        // FIX: filter pakai nama lengkap, bukan keyword
        final filtered = ctrl.allStories
            .where((s) => s.region == name)
            .toList();
        final stories = filtered.isNotEmpty
            ? filtered
            : ctrl.allStories.take(6).toList();
